...
#else
#define FTP_SERVER_ADDRESS  IP_ADDRESS(192,168,1,16)
#define FTP_SERVER_ADDRESS_STRING     "192,168,1,16"
#endif
...
TX_SEMAPHORE Semaphore_IPaddr_acquired;
...
UINT MX_NetXDuo_Init(VOID *memory_ptr)
{
  ...
  if (tx_byte_allocate(byte_pool, (VOID **) &pointer,  IP_MEMORY_SIZE, TX_NO_WAIT) != TX_SUCCESS)
  {
    return TX_POOL_ERROR;
  }
  ...
  ret = nx_ip_create(&IpInstance, "Main Ip instance", NULL_ADDRESS, NULL_ADDRESS, &NetXPool, nx_driver_emw3080_entry,
                     pointer, IP_MEMORY_SIZE, DEFAULT_NETX_PRIORITY);
...
/*  ret = tx_thread_create(&AppMainThread, "App Main thread", App_Main_Thread_Entry, 0, pointer, MAIN_THREAD_MEMORY,
  ...
  tx_semaphore_create(&Semaphore_IPaddr_acquired, "DHCP Semaphore", 0);
  tx_semaphore_create(&Semaphore_SNTP_ready, "SNTP ready Semaphore", 0);
#endif
...
static VOID ip_address_change_notify_callback(NX_IP *ip_instance, VOID *ptr)
{
  ...
  tx_semaphore_put(&Semaphore_IPaddr_acquired);
}
...
static VOID NetX_Thread_Entry(ULONG thread_input)
{
  ...
  if(tx_semaphore_get(&Semaphore_IPaddr_acquired, TX_WAIT_FOREVER) != TX_SUCCESS)
  {
    Error_Handler();
  }
  ...
  /* this thread is not needed any more, we terminate it */
...
ULONG Ftp_File_Get(CHAR *fname, FX_MEDIA * local_disk) {
    ...
    if (ret != NX_SUCCESS) 
    {
        printf("Failed to contact FTP server\n\r");
        /* Disconnect FTP client */
        nx_ftp_client_disconnect(&ftp_client, 5*NX_IP_PERIODIC_RATE);
        /* Delete the FTP clnt */
    ...
    printf("Created Local File %s file\n", fname);  
    /* Open the local module file.  */
    ret =  fx_file_open(local_disk, &my_file, fname, FX_OPEN_FOR_WRITE);

    if (ret != FX_SUCCESS) 
    {
      /* if local file is corrupted try to recover deleting it and recreating */
      if (ret == FX_FILE_CORRUPT)
      {
        ret = fx_file_delete(local_disk, fname);
        if (ret != FX_SUCCESS) return ret;
        ret =  fx_file_create(local_disk, fname);
        ret =  fx_file_open(local_disk, &my_file, fname, FX_OPEN_FOR_WRITE);
        if (ret != FX_SUCCESS) return ret;
      }
 ...
 //tx_thread_suspend(tx_thread_identify());   
    do
    {
       /* Read the remote file on FTP srv  */
       /* dont need to allocate a nx pkt as it allocated internally from the IP pkt pool by FTP clnt */      
       ...
       if (ret == NX_SUCCESS)
       {
          ...
          if (ret != FX_SUCCESS)
          {   /* release the FTP clnt allocated pkt */
              nx_packet_release(my_packet);              
              /* Close the local file.  */
              fx_file_close(&my_file);  
              /* Close remote FTP file */
              nx_ftp_client_file_close(&ftp_client, 5*NX_IP_PERIODIC_RATE);
              /* Disconnect FTP client */
              nx_ftp_client_disconnect(&ftp_client, 5*NX_IP_PERIODIC_RATE);
              /* Delete the FTP clnt */
              nx_ftp_client_delete(&ftp_client);              
