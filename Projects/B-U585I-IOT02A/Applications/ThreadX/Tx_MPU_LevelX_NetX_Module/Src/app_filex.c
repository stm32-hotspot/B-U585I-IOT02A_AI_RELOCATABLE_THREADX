...
static CHAR * media_buffer = FX_NULL;
...
/**
  ...
  * @param memory_ptr: memory media_buffer
  * @retval int
  */
UINT App_FileX_Init()
{
...
/** Initialize the FileX application  **/        
  ...
  ret = tx_byte_allocate(&fx_app_byte_pool, (VOID **)&media_buffer, FX_DISK_MEM_POOL_SIZE, TX_NO_WAIT);
  if (ret != TX_SUCCESS) {
    return ret;
  }
...
//#define FORCE_FLASH_FORMATTING   
#ifdef FORCE_FLASH_FORMATTING
    ret =  fx_media_format(&media_disk,
                            fx_stm32_levelx_nor_driver,   // Driver entry
                            (VOID*)LX_NOR_OSPI_DRIVER_ID,    // Device info media_buffer
                            (UCHAR *) media_buffer,       // Media buffer media_buffer
                            FX_SECTOR_SIZE,              // Media buffer size
...
//  ret =  fx_media_open(&media_disk, MEDIA_DISK_NAME, fx_stm32_levelx_nor_driver, (VOID*)LX_NOR_OSPI_DRIVER_ID, media_disk_sector_cache, sizeof(media_disk_sector_cache));
  ret =  fx_media_open(&media_disk, MEDIA_DISK_NAME, fx_stm32_levelx_nor_driver, (VOID*)LX_NOR_OSPI_DRIVER_ID, media_buffer, FX_DISK_MEM_POOL_SIZE);  
  if (ret != FX_SUCCESS) 
  {
  ...
  ret =  fx_media_space_available(&media_disk, &available_space);
  ...
  printf("User available NOR Flash disk space size : %lu bytes.\n", available_space);  
  return ret;
  ...
  /* volatile RAM disk need to be formatted at each reboot */
    ...
    ret = fx_media_exFAT_format(&media_disk,
                          ...
                          media_buffer,        // RAM disk memory pointer
    ...
    if (ret != FX_SUCCESS) {
      ...
      /* Open the fs disk.  */
    ret =  fx_media_open(&media_disk, MEDIA_DISK_NAME, _fx_ram_driver, media_buffer, media_disk_sector_cache, sizeof(media_disk_sector_cache));
    if (ret != FX_SUCCESS) {
      return ret;
    }  
    return ret;
...
UINT App_FileX_DeInit()
{
  ...
  tx_byte_release(media_buffer);   
  tx_byte_pool_delete(&fx_app_byte_pool);
  return FX_SUCCESS;
}

/* list current directory content: ls */
UINT App_FileX_List_Dir()
{
  CHAR file_name[FX_MAX_LONG_NAME_LEN];
  UINT ret = fx_directory_first_entry_find(&media_disk, &file_name[0]);
  printf  ("Local directory content:\n\r");
  while (ret == FX_SUCCESS)
  {     
    printf  ("          %s\n\r", file_name);
    ret = fx_directory_next_entry_find(&media_disk, &file_name[0]);
  }  
  return ret;
}

/* check the binary file signature (4bytes) against provided signature */
UINT App_FileX_Check_Binary_File(char * file_name, uint32_t file_signature)
{
  UINT ret = FX_SUCCESS;
  FX_FILE    my_file;
  
  /* check if the module file already exists in NOR FS  */
  ret =  fx_file_open(&media_disk, &my_file, file_name, FX_OPEN_FOR_READ);
  /* if file exist but is corrupted delete it */
  if (ret == FX_FILE_CORRUPT)
  {
    fx_file_delete(&media_disk, file_name);
    return ret;
  }
  ...
  /* Read and check the signature of the module file vs passed file_signature  */
  uint32_t signature;
  ULONG actual_size=0;      
  if (file_signature == NULL) 
  {  /* no reference signature provided: skip signature check */    
    fx_file_close(&my_file);    
    return FX_SUCCESS;
  }
  
  fx_file_seek(&my_file, 0);
  ret =  fx_file_read(&my_file, (VOID *) &signature, sizeof(file_signature), &actual_size);   
  if (ret != FX_SUCCESS || actual_size != sizeof(file_signature) || signature != file_signature)
  {
    fx_file_close(&my_file);
